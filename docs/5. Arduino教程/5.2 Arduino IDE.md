...
**特别提醒：本教程采用的 Arduino IDE 版本是 2.3.9 。如果是其他版本的话，不能保证本教程提供的示例代码能编译和上传成功。** 

### 5.2.2.1 Arduino IDE下载 
...
![a45_1](../img/Arduino_2.3.6-a44.png)

### 5.2.7.2 如何安装库文件

在这里，我们将为您介绍最简单的添加库的方法。我们是以添加Adafruit_BuslO库文件为例。

1\. 首先，依次点击左上角的 “**项目” > “导入库” > “添加 .Zip 库...”**

![a45](../img/Arduino_2.3.6-a45.png)

2\. 导航到库文件所在的目录（在 `资料下载` 中下载的ZIP文件解压，这里是以资料下载到电脑桌面上为例），打开文件夹 `Arduino_库文件`，然后选择`Adafruit_BusIO.zip`文件，点击 “**打开**” 按钮。

![a46](../img/Arduino_2.3.6-a46.png)

3\. 安装完成后，你将收到一条通知（已从Adafruit_BuslO.zip存档成功安装库），同时输出框会显示 “**Library installed**”，确认该库已成功添加到Arduino IDE中。下次需要使用此库时，你不需要重复安装过程。

![a47](../img/Arduino_2.3.6-a47.png)

4\. 重复前面相同的添加过程(方法)来添加其他(剩余)的库文件，这里就不一一重复讲解了。

## 5.2.8 安装 ESP32 开发板(**重要**)
...
5\. 在开发板搜索框中，搜索 “**ESP32**”，点击安装 ESP32 3.2.1 版本的ESP32开发板固件，右下角可以看到开发板固件安装进度，等待几分钟安装完成。**安装过程中请保持网络稳定，如安装失败，请重复以上步骤，重新安装开发板固件即可。**

⚠️ **<span style="color: rgb(255, 76, 65);">注意：</span> 本教程使用的是 ESP32 3.2.1 版本的ESP32开发板固件，请保持一致，以免出现代码不兼容情况。**

![Img](../img/Arduino_2.3.6-a32-1.png)

![Img](../img/Arduino_2.3.6-a33.png)

⚠️ **<span style="color: rgb(255, 76, 65);">特别提醒：假如，由于网络问题实在是下载安装不了ESP32开发板固件，我们也提供有ESP32开发板固件的压缩包，ESP32开发板固件的压缩包下载地址：</span>** [https://pan.baidu.com/s/1s_uPC74hzqW3fhfkNxp7NQ?pwd=3017](https://pan.baidu.com/s/1s_uPC74hzqW3fhfkNxp7NQ?pwd=3017) 

**提取码：3017**

**ESP32开发板固件的压缩包下载后解压，把解压后的ESP32开发板固件文件夹按照以下路径添加。ESP32开发板固件一般需要存放于以下路径，才能说明ESP32开发板固件安装好。**
...
5\. 在开发板搜索框中，搜索 “**ESP32**”，点击安装ESP32 3.2.1 版本的ESP32开发板固件，右下角可以看到开发板安装进度，等待几分钟安装完成。**安装过程中请保持网络稳定，如安装失败，请重复以上步骤，重新安装开发板即可。**
