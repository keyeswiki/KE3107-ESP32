...
对于这款简单的LED灯模块，可以直接使用ESP32主板的高低电平来控制LED的亮灭。
...
外接电源，选择好正确的开发板板型（ESP32 Dev Module）和 适当的串口端口（COMxx），然后单击![cou0](../../img/cou0.png)按钮上传代码。代码上传成功后，你会看到白色LED 灯以 1 秒的间隔闪烁：

- 点亮 1 秒 → 熄灭 1 秒 → 点亮 1 秒 → 熄灭 1 秒 → 循环往复。

![dongtu03](../../img/dongtu03.gif)
...
- `analogRead()` 调用ESP32的ADC，返回光敏电阻传感器模块分压电路的输出电压经ADC转换后的数字值（0 ~ 4095）。

- `sensorValue` 是ADC对光敏电阻传感器模块模拟信号的量化结果，数值大小与光照强度成正比（光照越强，值越大）。
...
外接电源，选择好正确的开发板板型（ESP32 Dev Module）和 适当的串口端口（COMxx），然后单击![cou0](../../img/cou0.png)按钮上传代码。代码上传成功后，当光照低于阈值时点亮路灯；当光照高于阈值时关闭路灯，实现节能的效果。

![dongtu09](../../img/dongtu09.gif)
