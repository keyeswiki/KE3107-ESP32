...
先调节接收功率，将电位器顺时针调到尽头，然后逆时针慢慢往回调，微调使传感器上SLED灯介于刚刚亮与不亮之间的 <span style="color: rgb(10, 10, 200);">不亮</span> 状态。


![6002](../../img/6002.gif)


接着调节发射频率，同样将电位器顺时针调到尽头，然后逆时针慢慢往回调，微调使传感器上SLED灯介于刚刚亮与不亮之间的 <span style="color: rgb(10, 10, 200);">不亮</span> 状态。
...
外接电源，选择好正确的开发板板型（ESP32 Dev Module）和 适当的串口端口（COMxx），然后单击![cou0](../../img/cou0.png)按钮上传代码。代码上传成功后，单击Arduino IDE右上角的![Sarial](../../img/Sarial.png)，打开串口监视器，波特率设置为 `9600`：

![dongtu04](../../img/dongtu04.gif)

- 当未检测到障碍物时，输出 `1` 和 `未检测到障碍物`。

- 当检测到障碍物时，输出 `0` 和 `检测到障碍物`。
...
- 传感器相关：指定避障传感器引脚，并设置计数器变量（ `volatile` 确保中断安全）。
...
外接电源，选择好正确的开发板板型（ESP32 Dev Module）和 适当的串口端口（COMxx），然后单击![cou0](../../img/cou0.png)按钮上传代码。代码上传成功后，我们将篮球投入篮筐，进球时：
