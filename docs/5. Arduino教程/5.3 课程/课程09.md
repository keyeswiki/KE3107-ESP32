...
<span style="color: rgb(255, 76, 65);">**特别提醒：Arduino文件夹中也提供有对应的示例代码（9_1），示例代码是与ESP32开发板固件版本 3.2.1 对应的。如果ESP32开发板固件版本是其他版本，那不能保证示例代码适合。在前面的Arduino IED教程(安装 ESP32 开发板 部分内容)中，也是强调要求安装ESP32开发板固件版本 3.2.1 的。**</span>
...
若串口监视器无内容显示，用手指上的指甲尖按一下复位键，如下图所示(<span style="color: rgb(255, 76, 65);">不建议拆掉摄像头模块下来处理，用指甲尖按复位键比较容易按</span>)：
