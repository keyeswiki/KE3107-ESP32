...
让我们用 AHT20 温湿度传感器和 OLED显示屏模块，打造一个智能教室温湿度监测系统，实时显示环境数据，为师生创造一个舒适健康的学习环境！
...
外接电源，选择好正确的开发板板型（ESP32 Dev Module）和 适当的串口端口（COMxx），然后单击![cou0](../../img/cou0.png)按钮上传代码。代码上传成功后，单击Arduino IDE右上角的![Sarial](../../img/Sarial.png)，打开串口监视器（波特率设置为 `9600`），每 2 秒更新一次温度和湿度数据。
...
外接电源，选择好正确的开发板板型（ESP32 Dev Module）和 适当的串口端口（COMxx），然后单击![cou0](../../img/cou0.png)按钮上传代码。代码上传成功后，在 OLED 屏幕上可以看到打印的 “Hello world!”：
...
外接电源，选择好正确的开发板板型（ESP32 Dev Module）和 适当的串口端口（COMxx），然后单击![cou0](../../img/cou0.png)按钮上传代码。代码上传成功后，AHT20传感器每2秒读取一次温湿度数值，并显示在OLED屏上。

![dongtu08](../../img/dongtu08.gif)
