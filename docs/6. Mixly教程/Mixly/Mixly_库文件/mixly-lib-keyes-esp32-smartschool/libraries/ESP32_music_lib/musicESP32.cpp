...
void music::sing(int s)
{
  if (s == 2) {
    ...
    for (int thisNote = 0; thisNote < size; thisNote++) {
      ...
      tone(buzzer_pin, underworld_melody_mario[thisNote], noteDuration);
    ...
    for (int thisNote = 0; thisNote < size; thisNote++) {
      ...
      tone(buzzer_pin, melody_mario[thisNote], noteDuration);
      int pauseBetweenNotes = noteDuration * 1.30;
      delay(pauseBetweenNotes);
      noTone(buzzer_pin);
...
void music::Ode_to_Joy(void)
{
  ...
  {
    tone(buzzer_pin,tune[x],300*durt[x]);
    delay(300*durt[x]*1.3);
  }
  noTone(buzzer_pin);  
}
/////////////////////////////////////////////
void music::christmas(void)
{
  ...
  {
    tone(buzzer_pin, tune_christmas[x],300*durt_christmas[x]);
    delay(300*durt_christmas[x]*1.3);        
    noTone(buzzer_pin);
  } 
